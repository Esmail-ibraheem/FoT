...
def pretrain(train_valid_test_dataset_provider,
    ...
    model, optimizer, lr_scheduler = setup_model_and_optimizer(model_provider, model_type="transformer")
...
def get_learning_rate_scheduler(optimizer):
    """Build the learning rate scheduler with dynamic parallelism support."""
    args = get_args()
    ...
    # Adjust warmup and decay steps based on parallel strategy
    if hasattr(optimizer.param_groups[0], 'current_dp_size'):
        dp_size = optimizer.param_groups[0].current_dp_size
        warmup_steps = warmup_steps // dp_size
        decay_steps = decay_steps // dp_size
...
def train_step(forward_step_func, data_iterator,
    ...
    # Update dynamic parallelism strategy if needed
    if hasattr(model, 'strategy_selector'):
        timers('dynamic-strategy-selection').start()
        model.strategy_selector.update_strategy()
        timers('dynamic-strategy-selection').stop()

    # Profile model and hardware metrics
    if hasattr(model, 'strategy_selector'):
        timers('profiling').start()
        model.strategy_selector.model_profiler.profile()
        model.strategy_selector.hardware_profiler.profile()
        timers('profiling').stop()
    ...
    # Log metrics to strategy selector
    if hasattr(model, 'strategy_selector'):
        model.strategy_selector.log_metrics(
            grad_norm=grad_norm,
            num_zeros_in_grad=num_zeros_in_grad,
            skipped_iter=skipped_iter,
            losses=losses_reduced
        )
    ...
    while iteration < args.train_iters:
        ...
        if args.curriculum_learning and \
            ...
            args.curriculum_seqlen = args.curriculum_scheduler.update_difficulty( \
                    args.iteration + 1)
        loss_dict, skipped_iter, grad_norm, num_zeros_in_grad = train_step(forward_step_func,
                              train_data_iterator,
                              model,
                              optimizer,
                              lr_scheduler)
