...
class DynamicStrategySelector:
    ...
    def __init__(self, model, metrics_history_size=100):
        self.model = model
    ...
    def select_strategy(self, metrics):
        """Select optimal parallelism strategy based on metrics."""
        # Add current metrics to history
        self.update_metrics(metrics)
        
        # Analyze metrics history to determine if strategy change needed
        if self._should_change_strategy():
            new_strategy = self._compute_optimal_strategy()
            if new_strategy != self.current_strategy:
                self._transition_to_strategy(new_strategy)
                self.current_strategy = new_strategy
                
    def _should_change_strategy(self):
        """Determine if strategy change is needed based on metrics."""
        if len(self.metrics_history) < 2:
            return False
            
        # Check for performance degradation
        recent_perf = self.metrics_history[-1]['throughput']
        avg_perf = sum(m['throughput'] for m in self.metrics_history[:-1]) / (len(self.metrics_history) - 1)
        
        return recent_perf < 0.9 * avg_perf  # 10% degradation threshold
        
    def _compute_optimal_strategy(self):
        """Compute optimal strategy based on current metrics."""
        metrics = self.metrics_history[-1]
        
        # Example logic - adjust based on memory and compute utilization
        if metrics['gpu_memory_util'] > 0.9:  # High memory pressure
            return self._increase_model_parallel()
        elif metrics['gpu_compute_util'] < 0.6:  # Low compute utilization
            return self._decrease_model_parallel()
            
        return self.current_strategy
        
    def _increase_model_parallel(self):
        """Increase model parallelism degree."""
        world_size = torch.distributed.get_world_size()
        new_strategy = self.current_strategy.copy()
        
        if new_strategy['tp_size'] * 2 <= world_size:
            new_strategy['tp_size'] *= 2
            new_strategy['dp_size'] = world_size // (new_strategy['tp_size'] * new_strategy['pp_size'])
            
        return new_strategy
        
    def _decrease_model_parallel(self):
        """Decrease model parallelism degree."""
        world_size = torch.distributed.get_world_size()
        new_strategy = self.current_strategy.copy()
        
        if new_strategy['tp_size'] > 1:
            new_strategy['tp_size'] //= 2
            new_strategy['dp_size'] = world_size // (new_strategy['tp_size'] * new_strategy['pp_size'])
            
        return new_strategy
        
    def _transition_to_strategy(self, new_strategy):
        """Transition to new parallelism strategy."""
        # Update model parallel state
        if hasattr(self.model, 'update_parallel_state'):
            self.model.update_parallel_state(
                tp_size=new_strategy['tp_size'],
                pp_size=new_strategy['pp_size']
            )
            
        # Reinitialize model parallel groups
        initialize_model_parallel(
            tensor_model_parallel_size_=new_strategy['tp_size'],
            pipeline_model_parallel_size_=new_strategy['pp_size']
        )
        
    def export_metrics_history(self):
