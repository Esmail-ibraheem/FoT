...
class Embedding(MegatronModule):
    """Language model embeddings with dynamic parallelism support.
    ...
    def update_parallel_state(self, tp_size=None):
        ...
        if tp_size is not None and tp_size != self.current_tp_size:
            ...
            # Reinitialize word embeddings for new parallel size
            args = get_args()
            self.word_embeddings = mpu.VocabParallelEmbedding(
                args.padded_vocab_size, self.hidden_size,
                init_method=self.init_method)
            
            # Position embeddings don't need update (serial)
            
            # Reinitialize token type embeddings if present
            if self.num_tokentypes > 0:
                self.tokentype_embeddings = torch.nn.Embedding(
                    self.num_tokentypes, self.hidden_size)
                self.init_method(self.tokentype_embeddings.weight)
...
class TransformerLanguageModel(MegatronModule):
    """Transformer language model with dynamic parallelism support.

    The architecture can be either:
    1) Embedding - Encoder - Pooler
    2) Embedding - Decoder
    3) Embedding - Encoder - Decoder
    """
    ...
    def update_parallel_state(self, tp_size=None, pp_size=None):
        ...
        # Update embedding parallel state
        if self.pre_process:
            self.embedding.update_parallel_state(tp_size)

        # Update encoder parallel state
        if self.encoder is not None:
            self.encoder.update_parallel_state(tp_size, pp_size)

        # Update decoder parallel state
        if self.decoder is not None:
            self.decoder.update_parallel_state(tp_size, pp_size)

        # Pooler doesn't need parallel state update (it's serial)
