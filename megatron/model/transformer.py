...
class ParallelAttention(MegatronModule):
    ...
    def update_parallel_state(self, tp_size=None):
        ...
        if tp_size and tp_size != mpu.get_tensor_model_parallel_world_size():
            self.hidden_size_per_partition = mpu.divide(
                self.query_key_value.weight.shape[1],
                tp_size)
            self.num_attention_heads_per_partition = mpu.divide(
                self.query_key_value.weight.shape[1],
                tp_size * self.hidden_size_per_attention_head * 3)
...
class ParallelTransformerLayer(MegatronModule):
    """A single transformer layer with dynamic parallelism support."""
    ...
    def update_parallel_state(self, tp_size=None, pp_size=None):
        """Update parallel state when strategy changes."""
        ...
        self.self_attention.update_parallel_state(tp_size)
        if self.layer_type == LayerType.decoder:
            self.inter_attention.update_parallel_state(tp_size)

        # Update MLP
        self.mlp.update_parallel_state(tp_size)

        # Reinitialize layer norm if tensor parallel size changes
        if tp_size is not None and tp_size != self.current_tp_size:
            args = get_args()
            self.input_layernorm = LayerNorm(
                args.hidden_size,
                eps=args.layernorm_epsilon)
    ...
    def _checkpointed_forward(self, hidden_states, attention_mask,
        ...
        if get_key_value:
            logger.warning('Caching not supported with activation checkpointing')

        if encoder_output is not None:
            inputs = [hidden_states, attention_mask, encoder_output, enc_dec_attn_mask, layer_past]
        else:
            inputs = [hidden_states, attention_mask, None, None, layer_past]

        return checkpoint(custom, *inputs)
